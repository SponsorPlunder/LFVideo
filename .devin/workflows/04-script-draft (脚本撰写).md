...
- **[口播]** 我不会写前端，但改一行配置，整条片就自动重渲了——这期就带你用大白话指挥 AI，把这套渲染引擎搭出来。
...
- **[画面]** 调用 `@TableScene` 模版。参数 headers=["路线", "适合干嘛", "不适合 / 已知坑"], rows=[["Remotion", "前端栈、跨期复用模板", "Node 打包阶段读浏览器对象会崩；BUSL 授权"], ["Manim", "数学/公式动画", "排版弱、渲染慢"]], highlightRow="0"
  - **[子镜头时间线]**（本段口播 >15s，必填）：0s 表头淡入 → 3s 第 1 行 stagger 入场 → 7s 第 2 行入场并高亮 → 12s Zoom 聚焦 Remotion 行 → 18s 缩表 + 浮出“为什么选它”结论卡
