...
- 分镜口播导演：`shared/roles/content/script-director(分镜口播导演).md`
  - 一体产出「画面（Remotion 组件映射/Props/动画/录屏 zoom 指令）+ 口播台词」的分镜口播稿（合并原视听策划师 + 文案撰稿人），并派生知乎/小红书改写稿。
  - 视觉字段用严谨工程术语、口播用《口播风格守则》两套子标准；基于 02 已确定的结构骨架写，不重新做选题判断。
