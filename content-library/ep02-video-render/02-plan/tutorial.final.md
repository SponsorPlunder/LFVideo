...
代价如实说：React 技术栈 + BUSL 授权（规模化商用要付费）。前端基础不用怕——本来就是 AI 写、我把方向。

---

## 四、技术落地：让 AI 把配置和现成组件对上号
...
### 3. 数字人：同样用 Vibe Coding 做选型与落地

视频要不要一个“出镜形象”做陪衬串场？这件事也没拍脑袋，套用和选渲染引擎一样的方法论——**先定位，再让 AI 列选型、说清坑，我对着约束拍板，最后让 AI 落地。**

**定位**：数字人只是**陪衬、串场**，不是主角；干货主体永远是屏幕上的真实操作。

**选型**（让 AI 把可选形象与各自的坑摆出来，`paper_spec`·决策性判断）：
...
**对约束拍板**：我的约束是不想露脸、要可编程批量复用、风格统一、还要避开恐怖谷——于是选 **3D 风格化角色 `VRMAvatar`**（`src/components/VRMAvatar.tsx`，`verified`），并把定位钉死：**陪衬、坚决不做对口型数字人、不做 AI 生成的假界面**，可信度只靠真实录屏。

**AI 落地**：让 AI 把它搭进引擎——

- **渲一次、按场景取景**：整体渲一次，再按场景裁角落/半身/全身（取景预设，`verified`），不用每场景重搭。
- **脚站得稳**：之前待机只摆髋部、整条腿像钟摆一样甩；修法是在大腿上把髋部摆动反向抵消，让脚踩原地、上半身自然摆（`verified`，见 PR「plant VRM feet」）。
- 口型、表情这些更重的能力留到以后单独一期（记进 `ideas/backlog.md`）。

---

## 五、总结

- **三步**：找路径（AI 罗列现成路线）→ 选型（AI 给"不适用+坑"，人对约束拍板）→ 落地（让 AI 把配置和组件对上号）。
- **为什么 Remotion**：模板换数据复用、AI 接手最稳、网页生态现成；代价是 React 栈 + BUSL 授权，不构成门槛。
- **引擎核心**：一份配置 → `Explainer` 按 `type` 分发现成组件；做内容 = 挑组件、填字段。
- **数字人也走同一套**：定位陪衬，让 AI 列真人/写实/二次元三种选型、对约束选定 VRM 3D 角色再落地，坚决不做对口型。
- **没编程基础也能复制**：会讲需求、对着约束拍板、把活交给 AI 即可。
...
> ⚠️ 本清单已按"找技术路径 → 技术选型 → 技术落地"的三段式重写；下游 03 分镜（README）与 04 脚本需按本清单同步重做。
...
### 三、技术选型
- [ ] 逼 AI 给"不适用 + 已知坑"的对比，人盯着坑做减法
- [ ] 回到自己的约束（固定模板批量复用 / 让 AI 接手 / 跨期维护）→ 为什么选 Remotion
- [ ] Remotion vs 复制粘贴 HTML 的对照；代价如实说（React 栈 + BUSL 授权），不渲染成门槛
...
- [ ] 数字人选型与落地：定位陪衬 → 让 AI 列真人出镜 / 写实对口型 / 二次元(VRM) 三种形象的适用与坑 → 对约束选定 VRM 3D 角色、定死不做对口型 → AI 落地（取景预设、脚站稳）
