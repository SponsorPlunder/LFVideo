...
- **[口播]** 先看结果：我改配置里这一行字，回车，新片一秒就出来了。靠的是一个判断——AI 最拿手的是文本和代码，那就把整条视频也写成代码和数据，改数据就等于改片。这一期要解决的，就是视频自动渲染。整套引擎我没手写一行，全程用大白话指挥 AI 搭出来，这就是 Vibe Coding：我没有前后端基础，只管把需求讲清楚、把结果判对错，写和填都交给它。路线就三步——先找技术路径，再做技术选型，最后落地出片。记一句：你说人话，AI 干脏活。
...
- **[口播]** 第一步是找路，别自己埋头翻文档——直接让 AI 把能拿代码生成视频的现成路子列全。摆出来你会发现，它们的底子其实一样：先用代码或数据描述画面，再让程序编译成一帧帧，最后拼成视频。
...
- **[口播]** AI 一口气给了六条。头一条是网页渲染，主力是 Remotion，用 React 组件加 CSS 把画面画出来，再让无头浏览器逐帧截图，复杂排版最在行。第二条 Motion Canvas，写函数描述动画时序，做代码讲解很顺。第三条 Manim，用 Python 描述几何和公式，是数学可视化的利器。第四条 MoviePy，Python 直接操作像素再配 FFmpeg，简单拼接够用。第五条 PixiJS 这类画布引擎，在 Canvas 上逐帧绘制，复杂粒子特效是强项。第六条 FFmpeg 加脚本，命令行合成，批量转码、烧字幕特别能打。名字五花八门，可扒开看都在做同一件事——把画面编译成帧。
...
- **[口播]** 第二步选型，也是最容易栽的一步。你要直接问 AI 哪个最好，它会把每个都夸一遍，等于没说。值钱的问法是逼它交代短板：每条路适合什么、在什么场景下不顶用、最关键的约束是什么，全列成这张表。先看 Remotion——它适合前端栈和复杂的自适应排版，但有两道硬约束：打包时如果在组件最外层读浏览器对象会直接崩，而且它走 BUSL 商业授权，规模化用要付费。其余几条也各有死穴：Motion Canvas 生态小、模板得自己攒；Manim 排版弱、大场景渲染慢；MoviePy 写自适应排版很繁琐、多层叠加吃内存；PixiJS 光是算文字对齐就够呛；FFmpeg 命令晦涩、出了问题极难定位。把坑摊在台面上，再对着自己的需求做减法。记一句：让 AI 把信息铺平，拍板的事你自己来。
...
- **[口播]** 坑摸清了，回到我自己的约束来拍板。我给 AI 摆了三条硬指标：一期定一个模板、换批数据就能批量出几十期；让 AI 接手改内容时尽量别出错；十几期下来还得撑得住维护。三条一比，Remotion 明显占优——组件和数据是分开的，改一处主题整个系列跟着变；每期只让 AI 填数据、套现成组件，最不容易翻车；出片一行命令就够；网页那套生态拿来就能用。说白了，它最听 AI 指挥，也最扛折腾。
...
- **[口播]** 也有人说，干脆让 AI 直接拿 HTML 拼得了。比一下就知道差在哪——注意，这两条路都是 AI 在写代码，真正的差距是工程化。第一是复用：Remotion 把组件和数据拆开，改一处全系列同步；裸 HTML 没有这层复用，AI 每期都得重新拼，越堆越乱。第二是约束：Remotion 用 TypeScript 把字段类型钉死，AI 只能规规矩矩填；裸 HTML 没有强类型兜底，结构很容易跑偏。第三是维护：到第十期，Remotion 还能稳稳往上扩，裸 HTML 早乱成一锅。代价也说清楚：得用 React 这套栈，还得认 BUSL、规模化付费。但前端反正是 AI 写、我把方向，正好把 Vibe Coding 的长处用满。一句话：规矩立在前头，比后面返工强。
...
- **[口播]** 选型定了，第三步落地，照样不用你手写，就是陪着 AI 把配置和组件对上号。仓库里这台引擎叫 remotion-composer：你写一份配置，说清这一段画面是什么、上面叠什么，主程序 Explainer 就照配置里的 type 字段，自动拎出对应组件去渲。type 写 comparison 就出对比卡，写 terminal_scene 是合成出来的终端、命令一行行打、不用真去录屏，写 screenshot_scene 就把截图叠上光标点击，再加上图表、概念图、分屏，全是现成通用件，照着填就行。想更有辨识度，还能让 AI 在这基础上扩一套自己的品牌组件库——那是更大的话题，留着以后单开一期。记一句：引擎只认 type，喂对就成。
...
- **[口播]** 用 Vibe Coding 做视频，最省心的就是这一招：从不让 AI 去发明新组件，只让它照现成组件填配置。比如想要一张对比卡，我就说一句——左边手工时序、右边声明式配置。它回给我的就是这段配置：type 填 comparison，标题、左右两栏的标签和内容填齐，完事。整个过程我一行代码没碰，只动了动嘴。记一句：别让 AI 造轮子，让它把现成轮子装上。
...
- **[口播]** 为什么填数据比写组件稳，看这组对照就明白。反面是让 AI 给这一期从零手写一个新组件，既在重复造轮子，又把换数据就复用这个最大的好处给丢了。正面是只让它产出一份数据，交给现成组件去渲。关键在于 Remotion 用 TypeScript 把每个 type 的字段格式焊死了，AI 一旦填错、漏填，编译当场报红，它只能在划好的格子里填空。这就像考试发的是填空题，而不是让你交一张白卷自由发挥——这也正是没基础也能把 AI 管住的原因。
...
- **[口播]** 想再配个出镜形象来串场？这事一样走选型那套流程。先把位置定死：数字人只是陪衬，不是主角。然后让 AI 把可选形象连同各自的坑一起摆出来——真人出镜最可信，可你得露脸，也没法编程批量复用；写实的对口型数字人，容易掉进恐怖谷，可信度反倒崩了；二次元的 3D 角色呢，风格统一、可编程、渲一次到处能用，还不踩恐怖谷。对着我的约束挑：不想露脸、要能批量复用、得避开恐怖谷——那就选 3D 角色 VRMAvatar，钉死它只做陪衬，坚决不做对口型。记一句：形象也让 AI 列，约束你来定。
...
- **[口播]** 选定之后，落地交给 AI。整体只渲一次，再按每段需要裁出角落、半身或者全身，不必一段一搭。早先待机时它只摆髋部，整条腿像钟摆一样来回甩；办法是在大腿上把这股摆动反向抵消掉，脚就稳稳踩在原地了。可信度始终靠真实录屏撑着，绝不整 AI 生成的假界面。
...
- **[口播]** 回头看就是三步：找路，让 AI 把现成路线全摆出来；选型，让 AI 把坑列清、你对着约束拍板；落地，填配置、套现成组件、再配个陪衬数字人，AI 自动出片。真正要练的从来不是写代码，而是把需求讲明白、把坑盯住、把活交给 AI——没基础也照样能抄。下期 EP03，用 Whisper 让字幕踩着话音、一个字一个字蹦出来。关注一下，别错过。
  ...
  "sections": [
    {
      ...
      "voice": "先看结果：我改配置里这一行字，回车，新片一秒就出来了。靠的是一个判断——AI 最拿手的是文本和代码，那就把整条视频也写成代码和数据，改数据就等于改片。这一期要解决的，就是视频自动渲染。整套引擎我没手写一行，全程用大白话指挥 AI 搭出来，这就是 Vibe Coding：我没有前后端基础，只管把需求讲清楚、把结果判对错，写和填都交给它。路线就三步——先找技术路径，再做技术选型，最后落地出片。记一句：你说人话，AI 干脏活。",
    ...
    },
    {
      ...
      "voice": "第一步是找路，别自己埋头翻文档——直接让 AI 把能拿代码生成视频的现成路子列全。摆出来你会发现，它们的底子其实一样：先用代码或数据描述画面，再让程序编译成一帧帧，最后拼成视频。",
    ...
    },
    {
      ...
      "voice": "AI 一口气给了六条。头一条是网页渲染，主力是 Remotion，用 React 组件加 CSS 把画面画出来，再让无头浏览器逐帧截图，复杂排版最在行。第二条 Motion Canvas，写函数描述动画时序，做代码讲解很顺。第三条 Manim，用 Python 描述几何和公式，是数学可视化的利器。第四条 MoviePy，Python 直接操作像素再配 FFmpeg，简单拼接够用。第五条 PixiJS 这类画布引擎，在 Canvas 上逐帧绘制，复杂粒子特效是强项。第六条 FFmpeg 加脚本，命令行合成，批量转码、烧字幕特别能打。名字五花八门，可扒开看都在做同一件事——把画面编译成帧。",
    ...
    },
    {
      ...
      "voice": "第二步选型，也是最容易栽的一步。你要直接问 AI 哪个最好，它会把每个都夸一遍，等于没说。值钱的问法是逼它交代短板：每条路适合什么、在什么场景下不顶用、最关键的约束是什么，全列成这张表。先看 Remotion——它适合前端栈和复杂的自适应排版，但有两道硬约束：打包时如果在组件最外层读浏览器对象会直接崩，而且它走 BUSL 商业授权，规模化用要付费。其余几条也各有死穴：Motion Canvas 生态小、模板得自己攒；Manim 排版弱、大场景渲染慢；MoviePy 写自适应排版很繁琐、多层叠加吃内存；PixiJS 光是算文字对齐就够呛；FFmpeg 命令晦涩、出了问题极难定位。把坑摊在台面上，再对着自己的需求做减法。记一句：让 AI 把信息铺平，拍板的事你自己来。",
    ...
    },
    {
      ...
      "voice": "坑摸清了，回到我自己的约束来拍板。我给 AI 摆了三条硬指标：一期定一个模板、换批数据就能批量出几十期；让 AI 接手改内容时尽量别出错；十几期下来还得撑得住维护。三条一比，Remotion 明显占优——组件和数据是分开的，改一处主题整个系列跟着变；每期只让 AI 填数据、套现成组件，最不容易翻车；出片一行命令就够；网页那套生态拿来就能用。说白了，它最听 AI 指挥，也最扛折腾。",
    ...
    },
    {
      ...
      "voice": "也有人说，干脆让 AI 直接拿 HTML 拼得了。比一下就知道差在哪——注意，这两条路都是 AI 在写代码，真正的差距是工程化。第一是复用：Remotion 把组件和数据拆开，改一处全系列同步；裸 HTML 没有这层复用，AI 每期都得重新拼，越堆越乱。第二是约束：Remotion 用 TypeScript 把字段类型钉死，AI 只能规规矩矩填；裸 HTML 没有强类型兜底，结构很容易跑偏。第三是维护：到第十期，Remotion 还能稳稳往上扩，裸 HTML 早乱成一锅。代价也说清楚：得用 React 这套栈，还得认 BUSL、规模化付费。但前端反正是 AI 写、我把方向，正好把 Vibe Coding 的长处用满。一句话：规矩立在前头，比后面返工强。",
    ...
    },
    {
      ...
      "voice": "选型定了，第三步落地，照样不用你手写，就是陪着 AI 把配置和组件对上号。仓库里这台引擎叫 remotion-composer：你写一份配置，说清这一段画面是什么、上面叠什么，主程序 Explainer 就照配置里的 type 字段，自动拎出对应组件去渲。type 写 comparison 就出对比卡，写 terminal_scene 是合成出来的终端、命令一行行打、不用真去录屏，写 screenshot_scene 就把截图叠上光标点击，再加上图表、概念图、分屏，全是现成通用件，照着填就行。想更有辨识度，还能让 AI 在这基础上扩一套自己的品牌组件库——那是更大的话题，留着以后单开一期。记一句：引擎只认 type，喂对就成。",
    ...
    },
    {
      ...
      "voice": "用 Vibe Coding 做视频，最省心的就是这一招：从不让 AI 去发明新组件，只让它照现成组件填配置。比如想要一张对比卡，我就说一句——左边手工时序、右边声明式配置。它回给我的就是这段配置：type 填 comparison，标题、左右两栏的标签和内容填齐，完事。整个过程我一行代码没碰，只动了动嘴。记一句：别让 AI 造轮子，让它把现成轮子装上。",
    ...
    },
    {
      ...
      "voice": "为什么填数据比写组件稳，看这组对照就明白。反面是让 AI 给这一期从零手写一个新组件，既在重复造轮子，又把换数据就复用这个最大的好处给丢了。正面是只让它产出一份数据，交给现成组件去渲。关键在于 Remotion 用 TypeScript 把每个 type 的字段格式焊死了，AI 一旦填错、漏填，编译当场报红，它只能在划好的格子里填空。这就像考试发的是填空题，而不是让你交一张白卷自由发挥——这也正是没基础也能把 AI 管住的原因。",
    ...
    },
    {
      ...
      "voice": "想再配个出镜形象来串场？这事一样走选型那套流程。先把位置定死：数字人只是陪衬，不是主角。然后让 AI 把可选形象连同各自的坑一起摆出来——真人出镜最可信，可你得露脸，也没法编程批量复用；写实的对口型数字人，容易掉进恐怖谷，可信度反倒崩了；二次元的 3D 角色呢，风格统一、可编程、渲一次到处能用，还不踩恐怖谷。对着我的约束挑：不想露脸、要能批量复用、得避开恐怖谷——那就选 3D 角色 VRMAvatar，钉死它只做陪衬，坚决不做对口型。记一句：形象也让 AI 列，约束你来定。",
    ...
    },
    {
      ...
      "voice": "选定之后，落地交给 AI。整体只渲一次，再按每段需要裁出角落、半身或者全身，不必一段一搭。早先待机时它只摆髋部，整条腿像钟摆一样来回甩；办法是在大腿上把这股摆动反向抵消掉，脚就稳稳踩在原地了。可信度始终靠真实录屏撑着，绝不整 AI 生成的假界面。",
    ...
    },
    {
      ...
      "voice": "回头看就是三步：找路，让 AI 把现成路线全摆出来；选型，让 AI 把坑列清、你对着约束拍板；落地，填配置、套现成组件、再配个陪衬数字人，AI 自动出片。真正要练的从来不是写代码，而是把需求讲明白、把坑盯住、把活交给 AI——没基础也照样能抄。下期 EP03，用 Whisper 让字幕踩着话音、一个字一个字蹦出来。关注一下，别错过。",
