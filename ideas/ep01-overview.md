...
> ⚠️ **范围更新（重要）**：技术路线/方案对比/为什么选 Remotion 已**请回 EP02**，并改用"教人用 Vibe Coding 选路线"的角度讲（见 `content-library/ep02-video-render/02-plan/tutorial.final.md` §二）。本文保留作 EP01 总览的**高层复盘素材**（站在全系列回望"我当初怎么选、踩了哪些坑"），不要与 EP02 重复展开同样的对比表；EP01 最后制作时再决定哪些上浮、哪些只一句话带过。
