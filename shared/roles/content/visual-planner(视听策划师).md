...
- **技术生态对齐**：精通 Remotion 组件库架构——模板场景/原语在 `OpenMontage/remotion-composer/src/custom-templates/`（`theme/`, `primitives/`, `scenes/` 分层），通用组件在 `OpenMontage/remotion-composer/src/components/`（以各自 `index.ts` 导出为准）。
...
### 第三段：用 Vibe Coding 选路线·对比矩阵（2:30-5:00）
- **组件**：`@TableScene`（承载多路线对比：路线 / 适用 / 不适用·已知坑）
- **Props**：完整填充对比数据（对齐 `tutorial.final.md` 的路线对比表与选型理由）...
...
|--------|--------|--------------|------------|----------|
| TICKET-epNN-01 | `CustomScene` | `OpenMontage/remotion-composer/src/custom-templates/scenes/CustomScene.tsx` | `interface CustomSceneProps { ... }` | ... |
